...
void CCharacter::HandleTiles(int Index)
{
	...
	if(!g_Config.m_SvOldTeleportHook && !g_Config.m_SvOldTeleportWeapons && z && !(*m_pTeleOuts)[z - 1].empty())
	{
		if(m_Core.m_Super)
			return;
		int TeleOut = g_Config.m_ClTeleportSeeded ? m_Core.m_pWorld->SeededRandomOr0((*m_pTeleOuts)[z - 1].size(), aSeed) : m_Core.m_pWorld->RandomOr0((*m_pTeleOuts)[z - 1].size());
	...
	if(evilz && !(*m_pTeleOuts)[evilz - 1].empty())
	{
		if(m_Core.m_Super)
			return;
		int TeleOut = g_Config.m_ClTeleportSeeded ? m_Core.m_pWorld->SeededRandomOr0((*m_pTeleOuts)[evilz - 1].size(), aSeed) : m_Core.m_pWorld->RandomOr0((*m_pTeleOuts)[evilz - 1].size());
	...
	if(Collision()->IsCheckEvilTeleport(MapIndex))
	{
		...
		for(int k = m_TeleCheckpoint - 1; k >= 0; k--)
		{
			...
			{
				int TeleOut = g_Config.m_ClTeleportSeeded ? m_Core.m_pWorld->SeededRandomOr0((*m_pTeleCheckOuts)[k].size(), aSeed) : m_Core.m_pWorld->RandomOr0((*m_pTeleCheckOuts)[k].size());
